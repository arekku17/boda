...
export default function Hero() {
  ...
  // Count down to the first event's start time in the event's time zone
  const firstEvent = config.agenda?.[0];
  const countdownTarget = firstEvent?.startTime
    ? `${config.date}T${firstEvent.startTime}:00${config.utcOffset || ""}`
    : config.date;
        ...
        >
          ...
          <div className={cn("space-y-4")}>
            ...
            <motion.h2
              variants={scaleIn}
              className={cn(
                "text-5xl sm:text-6xl font-script text-rose-600 leading-tight py-1",
              )}
            >
              {config.brideName}{" "}
            ...
            <motion.p
              variants={fade}
              className={cn(
                "text-gray-600 text-sm sm:text-base leading-relaxed max-w-sm mx-auto",
              )}
            >
              {config.description}
            </motion.p>
                ...
                {config.verse && (
                  <>
                    ...
                    <motion.div variants={fade} className={cn("space-y-2")}>
                      <p
                        className={cn(
                          "text-gray-600 font-serif italic text-lg leading-snug",
                        )}
                      >
                        “{config.verse.text}”
                      </p>
                      <p className={cn("text-rose-500 font-medium text-sm")}>
