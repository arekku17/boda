...
function App() {
  ...
  // Coming back from the gifts page skips the landing screen
  const openedFromGifts = Boolean(location.state?.openInvitation);
  const [isInvitationOpen, setIsInvitationOpen] = useState(openedFromGifts);
  ...
  // The visitor already interacted with the page, so music can resume
  useEffect(() => {
    if (openedFromGifts) audioControls.play();
      ...
      >
        <div className={cn("text-center max-w-md mx-auto p-6")}>
          ...
          <p className={cn("text-sm text-gray-500")}>
            Revisa el link o contacta a los organizadores.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <LanguageProvider language={activeConfig?.language || "es"}>
